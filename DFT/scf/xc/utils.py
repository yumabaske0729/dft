...
def compute_rho_grad_phi(basis_functions, D: np.ndarray, points: np.ndarray):
    ...
    前提: D は閉殻のスピン和密度 (D = 2P)。
      ρ(r)  = Σ_{μν} D_{μν} φ_μ φ_ν
      ∇ρ(r) = Σ_{μν} D_{μν} ( φ_ν ∇φ_μ + φ_μ ∇φ_ν )
      ※ 対角（μ=ν）は ∇(φ^2) = 2 φ ∇φ となるため、係数 2 が必須。
    ...
    for mu, gto in enumerate(basis_functions):
        ...
        for alpha, coef in zip(gto.exponents, gto.coefficients):
            ...
            lx = l * (x ** (l - 1)) * (y ** m) * (z ** n) if l > 0 else 0.0
            ly = m * (x ** l) * (y ** (m - 1)) * (z ** n) if m > 0 else 0.0
            lz = n * (x ** l) * (y ** m) * (z ** (n - 1)) if n > 0 else 0.0
    ...
    # ρ／∇ρ の初期化（※この後に初めて "+=" を使う）
    rho = np.zeros(N, dtype=np.float64)
    grad_rho = np.zeros((N, 3), dtype=np.float64)

    # 対角 μ=ν：∇(φ^2)=2 φ ∇φ → 係数 2
    for mu in range(nbf):
        p_mu = phi[mu]
        d_mm = float(D[mu, mu])
        if d_mm != 0.0:
            rho += d_mm * p_mu * p_mu
            grad_rho += 2.0 * d_mm * (p_mu[:, None] * grad_phi[mu])
...
def _accumulate_rho_grad_from_phi(
    ...
    前提: D は閉殻のスピン和密度 (D = 2P)。
      対角 μ=ν は ∇(φ^2)=2φ∇φ → 係数 2。
      非対角 μ<ν は c2 = 2 D_{μν}。
    """
    Nc = phi.shape[1]
    ...
    for m in diag_idx:
        ...
        p = phi[m]
        rho += d_mm * p * p
        grad_rho += 2.0 * d_mm * (p[:, None] * grad_phi[m])
    ...
    *,
    ...
    for blk in compute_rho_grad_phi_chunked(
        ...
        if return_energy_density:
            ...
            eps = energy_density_func(rho, grad)  # per-electron 等の定義は呼び出し側に合わせる
