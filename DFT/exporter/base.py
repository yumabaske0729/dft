...
class ExportOptions:
    ...
    （今回の改修では出力ルートは固定されるため、output_root や環境変数は参照されません）
    """
    # 既定ディレクトリ構成（ファイル名生成のため保持）
    base_out_dir: str = "DFT"
    include_subdir: str = "output"

    # タイムスタンプと run_id フォーマット
    timestamp_format: str = "%Y%m%d_%H%M%S"
    run_id_fmt: str = "{ts}_{input_tag}_{formula}_{basis}_{method}"  # basis は後で upper()

    # ファイル名フォーマット
    energies_csv_name_fmt: str = "energies_{input_tag}.csv"
    ...
    scf_csv_name_fmt: str = "scf_{input_tag}.csv"
    details_json_name_fmt: str = "details_{input_tag}.json"
...
class Exporter:
    ...
    def _resolve_output_root(self) -> Path:
        """
        出力ルートディレクトリを固定する。
        常に:  C:\\Users\\<USER>\\Desktop\\dft\\output
        """
        fixed = Path.home() / "Desktop" / "dft" / "output"
    ...
    def make_run_id_and_outdir(
        ...
        """
        run_id と保存先 outdir (= <fixed_output_root>/<run_id>) を返す
        """
        ...
        out_root = self._resolve_output_root()
        outdir = out_root / run_id
        return run_id, str(outdir)
    ...
    def export_final(
        ...
        *,
        ...
        matrices_json = os.path.join(outdir, self.opt.matrices_json_name_fmt.format(input_tag=input_tag))
