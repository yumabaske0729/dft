...
class IntegrationGrid:
    def __init__(self, points: np.ndarray, weights: np.ndarray, *, lebedev_degree: int, n_radial: int, rmax: float):
        ...
        self.meta = {
            "lebedev_order": int(lebedev_degree),  # 互換のためキー名維持（degree を格納）
...
def get_lebedev_grid(degree: int | None = None) -> tuple[np.ndarray, np.ndarray, int]:
    ...
    try:
        ...
        actual_deg = int(pts.shape[0])
        print(f"[grid] Lebedev degree request={req_deg}, actual={actual_deg}, N_ang={pts.shape[0]}")
        return pts, wts, actual_deg
